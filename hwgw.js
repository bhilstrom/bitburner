import {HackableBaseServer, numCycleForGrowthCorrected} from "./if.server.hackable"
...
export async function main(ns) {
	...
	while (growThreads > 0 || weakThreads1 > 0 || weakThreads2 > 0) {
		...
		if (home.threadCount(1.75) >= (growThreads + weakThreads1 + weakThreads2)) {
			if (weakThreads1 > 0) {
				ns.exec("weaken.js", "home", weakThreads1, target.id)
			}
			if (growThreads > 0) {
				ns.exec("grow.js", "home", growThreads, target.id)
			}
		} else {
			if (weakThreads1 > 0 && home.threadCount(1.75) > 0) {
				ns.exec("weaken.js", "home", Math.min(weakThreads1, home.threadCount(1.75)), target.id)
			} else if (growThreads > 0 && home.threadCount(1.75) > 0) {
				ns.exec("grow.js", "home", Math.min(growThreads, home.threadCount(1.75)), target.id)
			} else if (weakThreads2 > 0 && home.threadCount(1.75) > 0) {
				ns.exec("weaken.js", "home", Math.min(weakThreads2, home.threadCount(1.75)), target.id)
			}
		}
	...
	while (true) {
		...
		for (let target of targets) {
			...
			let wkSanityCheck = nextBatch.filter(batch => batch.filename == "weaken.js")
			let hkSanityCheck = nextBatch.filter(batch => batch.filename == "bin.hk.js")
			let grSanityCheck = nextBatch.filter(batch => batch.filename == "grow.js")
